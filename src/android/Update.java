...
public class Update {
	 ...
	 * @return
	 */
	public boolean getServerVer(String apkUrl,String versionName) {
...
//	 				versionNo = jsonObject.getString("appVersion");
//	 	 			updateUrl = jsonObject.getString("lastVersionOfApp");
//	        	 }else{
//	        		 return false;
//	        	 }
//	        	}
...
//			}
	...
	public void doNewVersionUpdate() {
		Log.i("out","is doNewVersionUpdate");
...
//		sb.append("M)");
	...
	private  boolean checkVersionNo(String local,String remote){
		if(local!=null){

			String[] remoteArray = remote.split("\\.");
			String[] localArray = local.split("\\.");


			for (int i = 0; i < 3; i++) {
				int iRemote = Integer.parseInt((remoteArray.length >= i + 1) ? remoteArray[i]
						: "0");
				int iLocal = Integer.parseInt((localArray.length >= i + 1) ? localArray[i]
						: "0");

				if (iRemote > iLocal) {
					return true;
				} else if (iLocal > iRemote) {
					return false;
				}
			}
			return false;
